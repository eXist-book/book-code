    ...
    <IndexTests>
        ...
        <RangeIndex repeat="{$repeats-range}" phrase="{$phrase-range}">
            <Description>Range index search for name elements with specific contents</Description>
            {
                ...
                return 
                    for $n in doc($doc-with-indexes)//tei:name[. eq $phrase-range]
                    return
                        $n
            ...
            }  
            {
                ...
                let $result  := for $i in 1 to $repeats-range
                return 
                    for $n in doc($doc-without-indexes)//name[. eq $phrase-range]
                    return
                        $n
                let $endtime as xs:time := util:system-time()
                return
        ...
        <NGramIndex repeat="{$repeats-ngram}" phrase="{$phrase-ngram}">
            ...
            {
                let $starttime as xs:time := util:system-time()
                let $result  := for $i in 1 to $repeats-ngram
                return 
                    for $n in doc($doc-with-indexes)//tei:p[ngram:contains(., $phrase-ngram)]
                    return
                        $n
                let $endtime as xs:time := util:system-time()
                return
            ...
            }
            {
                ...
                let $result  := for $i in 1 to $repeats-ngram
                return 
                    for $n in doc($doc-without-indexes)//p[contains(., $phrase-ngram)]
                    return
                        $n
                let $endtime as xs:time := util:system-time()
                return
        ...
        <FullTextIndex repeat="{$repeats-fulltext}" phrase="{$phrase-fulltext}">
            ...
            {
                let $starttime as xs:time := util:system-time()
                let $results := for $i in 1 to $repeats-fulltext
                return
